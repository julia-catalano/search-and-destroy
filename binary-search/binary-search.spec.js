...
describe('Binary Search', () => {
	it('returns true if target value is in array', () =>{
		expect(binarySearch(test1, 10, 3)).to.equal(true);
		expect(binarySearch(test2, -3, 5)).to.equal(true);
		expect(binarySearch(test2, 20, 3)).to.equal(true);
		expect(binarySearch(test3, 2.6, 2)).to.equal(true);
	});
